...
class PeftIDPGModelForMaskedLM(PeftIDPGModel):
	"""
	...
	**Attributes**:
	...
	def forward(
		...
		# [My Custom] Prepend soft prompt to the input embeddings
		if soft_prompt is None:
			raise ValueError("latent_attention_weights is required for IDPG-based prompt tuning")
	...
	def get_nb_trainable_parameters(self):
		...
		# Overwrite the trainable to zero for IDPG models
		trainable_params = 0
		return trainable_params, all_param


class PeftIDPGModelForFeatureExtraction(PeftIDPGModel):
	"""
	...
	**Attributes**:
	...
	def forward(
		...
		if peft_config.peft_type == PeftType.PREFIX_TUNING:
			...
			inputs_embeds = torch.cat((latent_prompt, inputs_embeds), dim=1)
			return self.base_model(inputs_embeds=inputs_embeds, **kwargs)
