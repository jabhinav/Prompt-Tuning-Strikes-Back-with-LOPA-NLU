...
def get_label(task, line):
	if task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "sst2", "CoLA"]:
		# GLUE style
...
def split_header(task, lines):
	...
	if task in ["cola", "subj", "mpqa", 'trec', 'mr']:
		return [], lines
	elif task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "sst2"]:
...
def main():
	...
	parser.add_argument("--task", type=str, nargs="+", default=["MNLI", "QQP", "RTE", "QNLI", "MRPC", "SNLI"],
						help="Task names")
	parser.add_argument("--seed", type=int, nargs="+", default=[100, 13, 21, 42, 87],
						help="Random seeds")
	parser.add_argument("--data_dir", type=str, default="../datasets/glue_data",
						help="Path to original data")
	parser.add_argument("--output_dir", type=str, default="../datasets/glue_data_low_resource",
						help="Output path")
	parser.add_argument("--mode", type=str, default='k-shot-10x', choices=['k-shot', 'k-shot-10x'],
	...
	for seed in args.seed:
		...
		for task, dataset in datasets.items():
			...
			selected_num_per_class_dev = [round(args.k_for_dev * (n - selected_num_per_class[i]) / (total_num - args.k))
										  for i, n in enumerate(num_per_class[0:-1])]
			...
			for name in names:
				with open(os.path.join(task_dir, name), "w") as f:
					...
					for line in train_header:
						writer.writerow(line)
					for label, k_per_class_dev, k_per_class in zip(label_list, selected_num_per_class_dev,
																   selected_num_per_class):
